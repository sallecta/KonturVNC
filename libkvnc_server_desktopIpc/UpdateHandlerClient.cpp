  ...
  m_log(log)
{
  ...
  lkvnc_rfb_PixelFormat termPF;
  Dimension termDim;
  getScreenProperties(&termPF, &termDim);

  m_backupFrameBuffer.setProperties(&termDim, &termPF);

  // Synchronize our lkvnc_rfb_FrameBuffer and the server lkvnc_rfb_FrameBuffer
  sendInit(m_forwGate);
}
...
void UpdateHandlerClient::extract(UpdateContainer *updateContainer)
{
  ...
  try {
    ...
    if (updCont.screenSizeChanged) {
      ...
      lkvnc_rfb_PixelFormat oldPf = m_backupFrameBuffer.getPixelFormat();
      Dimension oldDim = m_backupFrameBuffer.getDimension();
      // Get new screen properties
      lkvnc_rfb_PixelFormat newPf;
    ...
    updCont.lkvnc_rfb_CursorShapeChanged = m_forwGate->readUInt8() != 0;
    if (updCont.lkvnc_rfb_CursorShapeChanged) {
      m_log->info(_T("UpdateHandlerClient: cursor shape changed"));
      lkvnc_rfb_PixelFormat newPf = m_backupFrameBuffer.getPixelFormat();
      Dimension newDim = readDimension(m_forwGate);
...
void UpdateHandlerClient::getScreenProperties(lkvnc_rfb_PixelFormat *pf, Dimension *dim)
{
  AutoLock al(m_forwGate);
...
void UpdateHandlerClient::sendInit(BlockingGate *gate)
{
  ...
  lkvnc_rfb_PixelFormat tmpPixelFormat=m_backupFrameBuffer.getPixelFormat();
