...
void UpdateHandlerServer::extractReply(BlockingGate *backGate)
{
  ...
  const lkvnc_rfb_FrameBuffer *fb = m_updateHandler->getFrameBuffer();

  lkvnc_rfb_PixelFormat newPf = fb->getPixelFormat();
  ...
  backGate->writeUInt8(updCont.lkvnc_rfb_CursorShapeChanged);
  if (updCont.lkvnc_rfb_CursorShapeChanged) {
    const lkvnc_rfb_CursorShape *curSh = m_updateHandler->getCursorShape();
...
{
  const lkvnc_rfb_FrameBuffer *fb = m_updateHandler->getFrameBuffer();
  lkvnc_rfb_PixelFormat tmpPixelFormat = fb->getPixelFormat();
