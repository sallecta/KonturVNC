...
#include "../libkvnc_all_winSystem/DibSection.h"

// This class is a wrapper for a FramBuffer and a DIB section.
// It changes DIB section proerties by oneself according to lkvnc_rfb_FrameBuffer
// properties (such as width, height and lkvnc_rfb_PixelFormat)
class DibFrameBuffer : public lkvnc_rfb_FrameBuffer
{
public:
  DibFrameBuffer();
  ...
  virtual bool copyFrom(const lkvnc_rfb_FrameBuffer *srcFrameBuffer,
                        int srcX, int srcY);
  virtual bool overlay(const Rect *dstRect, const lkvnc_rfb_FrameBuffer *srcFrameBuffer,
  ...
  virtual inline lkvnc_rfb_PixelFormat getPixelFormat() const;

  // This function must uses instead of function that can change the lkvnc_rfb_FrameBuffer properties
  ...
  virtual void setProperties(const Dimension *newDim,
    const lkvnc_rfb_PixelFormat *lkvnc_rfb_PixelFormat, HWND compatibleWindow);
...
private:
  ...
  // lkvnc_rfb_FrameBuffer class and can't to be use in here. Also, if user code will to try
...
private:
  // This function updates a DIB section in accord with the lkvnc_rfb_FrameBuffer
