...
#include <crtdbg.h>
#include "lkvnc_rfb_KeySym.h"
#include "../libkvnc_all_util/CommonHeader.h"

#define lkvnc_rfb_DefsKeySym_MISCELLANY
#include "lkvnc_rfb_DefsKeySym.h"

lkvnc_rfb_KeySym::lkvnc_rfb_KeySym(RfbKeySymListener *extKeySymListener, LogWriter *log)
...
lkvnc_rfb_KeySym::~lkvnc_rfb_KeySym()
{
}

void lkvnc_rfb_KeySym::sendModifier(unsigned char virtKey, bool down)
{
  UINT32 rfbSym;
...
void lkvnc_rfb_KeySym::processKeyEvent(unsigned short virtKey,
                                unsigned int addKeyData)
{
  ...
  if (m_keyMap.virtualCodeToKeySym(&rfbSym, virtKey & 255)) {
    ...
    if (rfbSym == lkvnc_rfb_DefsKeySym_Return && extended) {
      rfbSym = lkvnc_rfb_DefsKeySym_KP_Enter;
    }
...
void lkvnc_rfb_KeySym::processCharEvent(WCHAR charCode,
                                 unsigned int addKeyData)
{
...
void lkvnc_rfb_KeySym::processFocusRestoration()
{
  m_log->info(_T("Process focus restoration in the lkvnc_rfb_KeySym class"));
...
void lkvnc_rfb_KeySym::processFocusLoss()
{
  m_log->info(_T("Process focus loss in the lkvnc_rfb_KeySym class"));
...
void lkvnc_rfb_KeySym::sendCtrlAltDel()
{
  ...
  sendVerbatimKeySymEvent(lkvnc_rfb_DefsKeySym_Control_L, true);
  sendVerbatimKeySymEvent(lkvnc_rfb_DefsKeySym_Alt_L, true);
  sendVerbatimKeySymEvent(lkvnc_rfb_DefsKeySym_Delete, true);
  sendVerbatimKeySymEvent(lkvnc_rfb_DefsKeySym_Delete, false);
  sendVerbatimKeySymEvent(lkvnc_rfb_DefsKeySym_Alt_L, false);
  sendVerbatimKeySymEvent(lkvnc_rfb_DefsKeySym_Control_L, false);
...
void lkvnc_rfb_KeySym::setWinKeyIgnore(bool winKeyIgnore)
{
  m_winKeyIgnore = winKeyIgnore;
}

void lkvnc_rfb_KeySym::releaseModifiers()
{
...
void lkvnc_rfb_KeySym::restoreModifiers()
{
...
void lkvnc_rfb_KeySym::releaseModifier(unsigned char modifier)
{
  UINT32 rfbSym;
...
void lkvnc_rfb_KeySym::releaseMeta()
{
  if (m_leftMetaIsPressed) {
    sendKeySymEvent(lkvnc_rfb_DefsKeySym_Meta_L, false);
  }
  if (m_rightMetaIsPressed) {
    sendKeySymEvent(lkvnc_rfb_DefsKeySym_Meta_R, false);
  }
}

void lkvnc_rfb_KeySym::restoreMeta()
{
  if (m_leftMetaIsPressed) {
    sendKeySymEvent(lkvnc_rfb_DefsKeySym_Meta_L, true);
  }
  if (m_rightMetaIsPressed) {
    sendKeySymEvent(lkvnc_rfb_DefsKeySym_Meta_R, true);
  }
}

void lkvnc_rfb_KeySym::restoreModifier(unsigned char modifier)
{
  UINT32 rfbSym;
  if (isPressed(modifier)) {
    bool success = m_keyMap.virtualCodeToKeySym(&rfbSym, modifier);
...
void lkvnc_rfb_KeySym::checkAndSendDiff(unsigned char virtKey, unsigned char state)
{
...
void lkvnc_rfb_KeySym::clearKeyState()
{
  memset(m_viewerKeyState, 0, sizeof(m_viewerKeyState));
  memset(m_serverKeyState, 0, sizeof(m_serverKeyState));
}

bool lkvnc_rfb_KeySym::isPressed(unsigned char virtKey)
{
  return (m_serverKeyState[virtKey] & 128) != 0;
}

void lkvnc_rfb_KeySym::sendKeySymEvent(unsigned int lkvnc_rfb_KeySym, bool down)
{
  // Translate Alt to Meta if Scroll Lock is on.
  if (lkvnc_rfb_KeySym == lkvnc_rfb_DefsKeySym_Alt_L || lkvnc_rfb_KeySym == lkvnc_rfb_DefsKeySym_Alt_R) {
    bool scrollLockOn = (GetKeyState(VK_SCROLL) & 1) != 0;
    if (scrollLockOn) {
      if (lkvnc_rfb_KeySym == lkvnc_rfb_DefsKeySym_Alt_L) {
        lkvnc_rfb_KeySym = lkvnc_rfb_DefsKeySym_Meta_L;
        m_leftMetaIsPressed = down;
      } else { // assuming (lkvnc_rfb_KeySym == lkvnc_rfb_DefsKeySym_Alt_R)
        lkvnc_rfb_KeySym = lkvnc_rfb_DefsKeySym_Meta_R;
  ...
  sendVerbatimKeySymEvent(lkvnc_rfb_KeySym, down);
}

void lkvnc_rfb_KeySym::sendVerbatimKeySymEvent(unsigned int lkvnc_rfb_KeySym, bool down)
{
  m_extKeySymListener->onRfbKeySymEvent(lkvnc_rfb_KeySym, down);
}

unsigned char lkvnc_rfb_KeySym::distinguishLeftRightModifier(unsigned char virtKey,
                                                      bool isRightHint)
{
