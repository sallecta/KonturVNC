...
#include "lkvnc_rfb_PixelConverter.h"
#include "../libkvnc_all_util/inttypes.h"
#include <crtdbg.h>

lkvnc_rfb_PixelConverter::lkvnc_rfb_PixelConverter(void)
...
void lkvnc_rfb_PixelConverter::convert(const Rect *rect, lkvnc_rfb_FrameBuffer *dstFb,
                             const lkvnc_rfb_FrameBuffer *srcFb) const
{
  if (m_convertMode == NO_CONVERT) {
    ...
    lkvnc_rfb_PixelFormat dstPf = dstFb->getPixelFormat();
    lkvnc_rfb_PixelFormat srcPf = srcFb->getPixelFormat();

    UINT32 dstPixelSize = dstPf.bitsPerPixel / 8;
    UINT32 srcPixelSize = srcPf.bitsPerPixel / 8;

    // FIXME: Make lkvnc_rfb_FrameBuffer do the math.
...
const lkvnc_rfb_FrameBuffer *
lkvnc_rfb_PixelConverter::convert(const Rect *rect, const lkvnc_rfb_FrameBuffer *srcFb)
{
  if (m_convertMode == NO_CONVERT) {
    return srcFb;
  }
  ...
  if (m_dstFrameBuffer == 0) {
    ...
    m_dstFrameBuffer = new lkvnc_rfb_FrameBuffer;
...
void lkvnc_rfb_PixelConverter::reset()
{
  // Deallocate the lkvnc_rfb_FrameBuffer.
  if (m_dstFrameBuffer != 0) {
...
void lkvnc_rfb_PixelConverter::setPixelFormats(const lkvnc_rfb_PixelFormat *dstPf,
...
size_t lkvnc_rfb_PixelConverter::getSrcBitsPerPixel() const
{
  return m_srcFormat.bitsPerPixel;
}

size_t lkvnc_rfb_PixelConverter::getDstBitsPerPixel() const
{
  return m_dstFormat.bitsPerPixel;
}

void lkvnc_rfb_PixelConverter::fillHexBitsTable(const lkvnc_rfb_PixelFormat *dstPf,
                                      const lkvnc_rfb_PixelFormat *srcPf)
{
...
void lkvnc_rfb_PixelConverter::fill32BitsTable(const lkvnc_rfb_PixelFormat *dstPf,
                                     const lkvnc_rfb_PixelFormat *srcPf)
{
  UINT32 dstRedMax = dstPf->redMax;
  UINT32 dstGrnMax = dstPf->greenMax;
...
UINT32 lkvnc_rfb_PixelConverter::rotateUint32(UINT32 value) const
{
  UINT32 result;
  char *src = (char *)&value;
