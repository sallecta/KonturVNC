...
#include "lkvnc_rfb_KeySymListener.h"
#include "../libkvnc_all_util/Keymap.h"
...
class lkvnc_rfb_KeySym
{
public:
  lkvnc_rfb_KeySym(RfbKeySymListener *extKeySymListener, LogWriter *log);
  virtual ~lkvnc_rfb_KeySym();
...
private:
  ...
  virtual void sendKeySymEvent(unsigned int lkvnc_rfb_KeySym, bool down);

  // Send one key event (Alt not translated to Meta).
  virtual void sendVerbatimKeySymEvent(unsigned int lkvnc_rfb_KeySym, bool down);
