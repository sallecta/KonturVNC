...
class lkvnc_rfb_CursorShape
{
public:
  lkvnc_rfb_CursorShape(): m_mask(0) {}
  ~lkvnc_rfb_CursorShape();

  bool clone(const lkvnc_rfb_CursorShape *srcCursorShape);

  bool setDimension(const Dimension *newDim);
  Dimension getDimension() const { return m_pixels.getDimension(); }

  bool setPixelFormat(const lkvnc_rfb_PixelFormat *lkvnc_rfb_PixelFormat);
  lkvnc_rfb_PixelFormat getPixelFormat() const { return m_pixels.getPixelFormat(); }

  // This function set both lkvnc_rfb_PixelFormat and Dimension
  bool setProperties(const Dimension *newDim, const lkvnc_rfb_PixelFormat *lkvnc_rfb_PixelFormat);

  const lkvnc_rfb_FrameBuffer *getPixels() const { return &m_pixels; }
...
private:
  ...
  lkvnc_rfb_FrameBuffer m_pixels;
