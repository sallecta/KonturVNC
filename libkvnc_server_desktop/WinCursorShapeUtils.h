...
public:
  ...
  static void fixAlphaChannel(const lkvnc_rfb_FrameBuffer *pixels, char *maskAND, bool isInversedAlpha);
  ...
  static void winMonoShapeToRfb(const lkvnc_rfb_FrameBuffer *pixels, char *maskAND, char *maskXOR);
  ...
  template< typename T >
  static bool winColorShapeToRfb(const lkvnc_rfb_FrameBuffer *pixels, char *maskAND);
...
private:
  ...
  static UINT32 getAlphaMask(const lkvnc_rfb_PixelFormat *pf);
};

template< typename T >
bool WinCursorShapeUtils::winColorShapeToRfb(const lkvnc_rfb_FrameBuffer *pixels,
                                                    char *maskAND)
{
  char *pixelsBuffer = (char *)pixels->getBuffer();
  lkvnc_rfb_PixelFormat pf = pixels->getPixelFormat();
  T *pixel;
