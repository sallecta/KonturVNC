...
bool WindowsCursorShapeGrabber::grab(const lkvnc_rfb_PixelFormat *lkvnc_rfb_PixelFormat)
{
  return grabPixels(lkvnc_rfb_PixelFormat);
}

bool WindowsCursorShapeGrabber::grabPixels(const lkvnc_rfb_PixelFormat *lkvnc_rfb_PixelFormat)
{
  HCURSOR hCursor = getHCursor();
  if (hCursor == 0) {
    return false;
  ...
  const lkvnc_rfb_FrameBuffer *pixels= m_cursorShape.getPixels();

  Dimension tmpDimension = Dimension(width, height);
  m_cursorShape.setProperties(&tmpDimension, lkvnc_rfb_PixelFormat);
  ...
  bmi.bmiHeader.biBitCount = lkvnc_rfb_PixelFormat->bitsPerPixel;
  bmi.bmiHeader.biWidth = width;
  bmi.bmiHeader.biHeight = -height;
  bmi.bmiHeader.biCompression = BI_BITFIELDS;
  bmi.red   = lkvnc_rfb_PixelFormat->redMax   << lkvnc_rfb_PixelFormat->redShift;
  bmi.green = lkvnc_rfb_PixelFormat->greenMax << lkvnc_rfb_PixelFormat->greenShift;
  bmi.blue  = lkvnc_rfb_PixelFormat->blueMax  << lkvnc_rfb_PixelFormat->blueShift;
