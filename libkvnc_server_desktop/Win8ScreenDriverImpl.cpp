...
#include <crtdbg.h>
#include "../libkvnc_all_winSystem/Screen.h"
...
lkvnc_rfb_FrameBuffer *Win8ScreenDriverImpl::getScreenBuffer()
{
...
void Win8ScreenDriverImpl::initDxgi()
{
  ...
  // Desktop Duplication API here rather than getting the wrong lkvnc_rfb_FrameBuffer.
  Screen screen;
  if (screen.getVisibleMonitorCount() != dxgiOutputArray.size()) {
    throw Exception(_T("Unable get all DXGI outputs for virtual screen"));
  }

  lkvnc_rfb_PixelFormat pf = getDxPixelFormat();
...
lkvnc_rfb_PixelFormat Win8ScreenDriverImpl::getDxPixelFormat() const
{
  return lkvnc_rfb_PixelFormatCreator::create32bppPixelFormat();
}

void Win8ScreenDriverImpl::updateCursorShape(lkvnc_rfb_CursorShape *dst)
{
  AutoLock al(&m_cursorMutex);
