...
public:
  ...
  // Provides lkvnc_rfb_FrameBuffer grabbing.
  // Parameters:     *rect - Pointer to a Rect object with relative workRect coordinates.
  // Return value:   true if success.
  // Implementions will ensure that this function is thread safety.
  virtual bool grabFb(const Rect *rect = 0) = 0;

  // Returns a pointer an internal screen driver lkvnc_rfb_FrameBuffer
  virtual lkvnc_rfb_FrameBuffer *getScreenBuffer() = 0;
  ...
  // Implementions will ensure that this function is thread safety.
  virtual bool grabCursorShape(const lkvnc_rfb_PixelFormat *pf) = 0;

  // Returns a pointer an internal screen driver lkvnc_rfb_CursorShape
  virtual const lkvnc_rfb_CursorShape *getCursorShape() = 0;
