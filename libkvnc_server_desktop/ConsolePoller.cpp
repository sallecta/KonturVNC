...
ConsolePoller::ConsolePoller(UpdateKeeper *updateKeeper,
                             ...
                             lkvnc_rfb_FrameBuffer *backupFrameBuffer,
                             LocalMutex *lkvnc_rfb_FrameBufferMutex,
                             LogWriter *log)
: UpdateDetector(updateKeeper, updateListener),
  m_screenGrabber(screenGrabber),
  m_backupFrameBuffer(backupFrameBuffer),
  m_frameBufferMutex(lkvnc_rfb_FrameBufferMutex),
  m_log(log)
{
  ...
  while (!isTerminating()) {
    ...
    if (!conRect.isEmpty()) {
...

      {
        ...
        lkvnc_rfb_FrameBuffer *screenFrameBuffer = m_screenGrabber->getScreenBuffer();
